...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for ravend is necessary for this upgrade with the "reindex=1" raven.conf option. \n' +
                 'There are changes necessary in both raven.conf and ravencore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/dataturk/ravencore-node/blob/master/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed ravencore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './ravencore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in ravencore-node services directory
    ...
    if (servicePackage.ravencoreNode) {
      serviceModule = service.name + '/' + servicePackage.ravencoreNode;
    }
